...
def before_first_request():
    # Joseph suggested to change " = " to update and that solved the problem of routs "not seeing" students_dict update
    # but that only works when the global students_dict initially is set to {} and not to None like Noam suggested
...
def add_student():
    ...
    # response = app.response_class(
    #     response=json.dumps(new_student),
    #     status=200,
    #     mimetype='application/json')
...
def delete_student():
    ...
    return f"Student with login {to_delete['email']} deleted successfully"

#
# @app.route("/users")
# def get_all_users():
#     users = data_layer.get_all_users()
#     resp = app.response_class(response=json.dumps({"users": users}))
#     return resp
